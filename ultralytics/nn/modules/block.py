...
class DFL(nn.Module):
    ...
    def __init__(self, c1=16):
        ...
        super().__init__()
        self.conv = nn.Conv2d(c1, 1, 1, bias=False).requires_grad_(False)  # 创建一个2D卷积层，输入通道数为c1，输出通道数为1，卷积核大小为 1x1（相当于全连接层）
        x = torch.arange(c1, dtype=torch.float)
        self.conv.weight.data[:] = nn.Parameter(x.view(1, c1, 1, 1))  # 张量x重塑为(1, c1, 1, 1)的形状，并作为卷积层的权重
        self.c1 = c1
