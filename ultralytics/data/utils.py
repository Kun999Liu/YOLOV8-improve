...
def check_det_dataset(dataset, autodownload=True):
    ...
    """
    file = check_file(dataset)  # 检查文件是否存在，如果不存在则尝试下载，最后返回文件的路径

    # Download (optional)
    extract_dir = ""
    if zipfile.is_zipfile(file) or is_tarfile(file):  # 如果文件是压缩文件，则下载并解压缩到指定目录
    ...
    # Resolve paths
    path = Path(extract_dir or data.get("path") or Path(data.get("yaml_file", "")).parent)  # 获取数据集根目录
    if not path.is_absolute():  # 检查当前的 path 是否是绝对路径
        path = (DATASETS_DIR / path).resolve()

    # Set paths  将train val test的相对路径变成绝对路径
    ...
    if val:
        ...
        if not all(x.exists() for x in val):  # 检查 val 中的每个路径是否存在
